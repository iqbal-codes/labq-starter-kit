...
- CRM features: overview, leads, contacts, companies, deals, pipeline, plus route-based detail pages under `/leads/:id`, `/contacts/:id`, etc.
...
- TanStack Form + shadcn/ui integration via `@labq-modules/ui/components/forms`
...
- CRM handlers enforce RBAC with `requirePermission(...)`
...
- Drizzle ORM
- 7 auth tables + 8 business tables (15 total) — leads, contacts, attachments, companies, dealStages, deals, crmActivities, organizationSettings, auditLogs
- 3 `pgEnum` types: `dealStageKindEnum`, `crmActivityTypeEnum`, `crmActivityEntityTypeEnum` (plus `moduleKeyEnum` retained in DB but not in Drizzle schema exports)
- CRM tables follow the full soft-delete / audit-column pattern (`deletedAt` + `createdBy` + `updatedBy`)
...
- CRM router enforces `crm.view`, `crm.create`, `crm.update`, `crm.delete` on its handlers
- CRM frontend gating uses `apps/web/src/hooks/use-permissions.ts`, which reads `orpc.organization.getContext` for the active permission array
...
- Schema contract: `CONTACT_IMPORT_EXPORT_FIELDS` in `@labq-modules/schemas` defines round-trippable CSV columns
...
- CRM list endpoints (`leads.list`, `contacts.list`, `companies.list`, `deals.list`, `contacts.exportRows`) expose a single `search: z.string().optional()` input instead of column-specific params
...
Backend search matches (via `ilike`): leads → name/email/companyName, contacts → name/email, companies → name, deals → title
